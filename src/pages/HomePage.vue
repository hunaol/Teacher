<!--
  HomePage.vue — 应用首页（带登录注册的着陆页）
  ====================================================
  视觉重塑后的首屏，集成品牌展示、动画字条、登录/注册三种教师身份切换。
  若用户已登录，页面将直接跳转到对应身份的工作区首页。
-->
<script setup>
    ...
    <section class="home-main" :class="{ 'hero-visible': heroVisible }">
      ...
      <div class="home-hero">
        ...
        <div class="hero-visual-bg">
          <div class="hero-float-card card-1">
            <FileText :size="18" /><span>AI 智能备课</span>
          </div>
          <div class="hero-float-card card-2">
            <TrendingUp :size="18" /><span>学情分析完成</span>
          </div>
          <div class="hero-float-card card-3">
            <Users :size="18" /><span>3,200+ 教师在线</span>
          </div>
          <div class="hero-float-card card-4">
            <Sparkles :size="18" /><span>错题智能诊断</span>
          </div>
          <div class="hero-float-card card-5">
            <FileText :size="18" /><span>课题研究导航</span>
          </div>
      ...
      <div class="home-login-card">
        ...
        <div class="role-cards">
          <button v-for="o in roleOptions" :key="o.key" type="button" class="role-card"
            :class="{ active: selectedRole === o.key }" :style="{ '--rc': o.color }" @click="selectedRole = o.key">
...
.hero-visual-core {
  ...
  justify-content: center;
  color: #fff;
  box-shadow: 0 6px 30px rgba(217, 140, 82, .3);
...
.hero-float-card {
  ...
  font-weight: 500;
  opacity: 0;
  animation: floatCardIn .6s ease-out forwards;
  white-space: nowrap;
  z-index: 1;
}
...
.card-4 {
  animation-name: floatCardIn2;
}

@keyframes floatCardIn2 {
  from {
    opacity: 0;
